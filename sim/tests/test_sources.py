...
class TestSources(unittest.TestCase):
    def test_constant(self):
        ...
        src = ConstantSource(0.1, dt = 0.1, duration = 5., voltage = 3.3)
    ...
    def test_sine(self):
        ...
        src = SineSource(0.1, 0.2, 60., 0., dt = 0.1, duration = 3.,
                         voltage = 2.5)

        self.assertAlmostEqual(src.get_voltage(), 2.5)

        # check power at two points
        self.assertAlmostEqual(src.get_power(0.), 0.2)
    ...
    def test_bonito_basic(self):
        ...
        data_path = os.path.join(script_dir, "..", "data", "bonito",
                                 "pwr_cars.h5")

        src = BonitoSource(data_path, voltage = 1.65)

        self.assertAlmostEqual(src.get_voltage(), 1.65)
        self.assertAlmostEqual(src.get_power(0.), 5.69625264568467e-06)
    ...
    def test_bonito_name(self):
        ...
        self.assertAlmostEqual(src.get_power(0.), 3.652359120783698e-06)

    def test_bonito_out_of_sync(self):
        """Check detection of out of sync events with sim"""
